...
const buildYourBoxController = {
  ...
  async getFrequentCombos(req, res) {
    try {
      const { days = 7, minSupport = 3 } = req.query;
  ...
  /**
   * DELETE /api/byb/funnel/reset
   * Reset all funnel data and invalidate snapshots
   */
  async resetFunnel(req, res) {
    try {
      console.log('🔄 API: Resetting BYB Funnel...');
      const result = await buildYourBoxService.resetFunnel();
      ...
      console.error('BYB Funnel Reset Error:', error);
      res.status(500).json({
        success: false,
        error: 'Error resetting funnel'
      });
    }
  },
