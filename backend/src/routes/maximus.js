...
router.put('/config', authorize('admin'), async (req, res) => {
  try {
    ...
    const {
      active, creativeAgentReady, lists,
      maxCampaignsPerWeek, maxCampaignsPerDay, minHoursBetweenSameDay,
    ...
    if (modelForAnalysis) config.modelForAnalysis = modelForAnalysis;
